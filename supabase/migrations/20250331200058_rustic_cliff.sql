/*
  ...
  1. New Tables
    - tiktok_videos: Store video metadata
...
DROP POLICY IF EXISTS "Users can view their connected TikTok videos" ON tiktok_videos;
DROP POLICY IF EXISTS "No direct access to webhook logs" ON tiktok_webhooks;
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_tiktok_videos_creator_id ON tiktok_videos(creator_id);
CREATE INDEX IF NOT EXISTS idx_tiktok_webhooks_event_type ON tiktok_webhooks(event_type);
CREATE INDEX IF NOT EXISTS idx_tiktok_webhooks_status ON tiktok_webhooks(status);